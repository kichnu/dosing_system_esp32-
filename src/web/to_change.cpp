        ...
        <div class="modal-overlay" id="refillModal">
            <div class="modal-box">
                ...
                <div class="modal-text">
                    Reset remaining volume to full container capacity?
                </div>
                <div class="modal-info">
                    <div class="modal-info-label">Channel <span id="modalChannel">1</span></div>
                    <div class="modal-info-value"><span id="modalCapacity">1000</span> ml</div>
                </div>
                <div class="modal-actions">
                    <button class="btn btn-secondary" onclick="closeRefillModal()">Cancel</button>
                    <button class="btn btn-primary" onclick="confirmRefill()">Refill</button>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="card-content">
                ...
                <div class="section">
                    ...
                    <div class="section-body">
                        <div class="volume-row">
                            <div class="volume-group">
                                <label class="volume-label">Container Size</label>
                                <input type="number" class="volume-input" id="container_${idx}" 
                                       value="${ch.containerMl || 1000}" step="50" min="100" max="5000"
                                       onchange="saveContainerSize(${idx})">
                            </div>
                            <span class="volume-unit">ml</span>
                        </div>
                        <div class="container-status">
                            <div class="container-bar">
                                <div class="container-bar-fill ${ch.lowVolume ? 'low' : ''}" 
                                     id="containerBar_${idx}"
                                     style="width: ${ch.remainingPct || 100}%"></div>
                                <div class="container-bar-text" id="containerText_${idx}">
                                    ${(ch.remainingMl || 1000).toFixed(0)} ml
                                </div>
                            </div>
                            <div class="container-pct ${ch.lowVolume ? 'low' : ''}" id="containerPct_${idx}">
                                ${ch.remainingPct || 100}%
                            </div>
                        </div>
                        <div class="calc-grid">
                            <div class="calc-item ${ch.lowVolume ? 'warn' : ''}">
                                <div class="calc-lbl">Remaining</div>
                                <div class="calc-val" id="remaining_${idx}">${(ch.remainingMl || 1000).toFixed(0)} ml</div>
                            </div>
                            <div class="calc-item">
                                <div class="calc-lbl">Days Left</div>
                                <div class="calc-val" id="daysLeft_${idx}">${ch.daysRemaining ? ch.daysRemaining.toFixed(1) : '∞'}</div>
                            </div>
                        </div>
                        <button class="refill-btn" onclick="showRefillModal(${idx})">
                            <svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                <path d="M12 2L2 7l10 5 10-5-10-5z"/>
                                <path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/>
                            </svg>
...
function saveContainerSize(idx) {
    ...
    fetch('api/container-volume', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ channel: idx, container_ml: clamped })
    })
    .then(r => r.json())
    .then(data => {
        if (data.success) {
            updateContainerDisplay(idx, data);
            console.log(`Container CH${idx} set to ${clamped} ml`);
        }
    })
    .catch(err => console.error('Save container error:', err));
}
...
function closeRefillModal() {
    ...
    pendingRefillChannel = -1;
}

function confirmRefill() {
    if (pendingRefillChannel < 0) {
        closeRefillModal();
        return;
    }
    ...
    fetch(`api/refill?channel=${channel}`, { method: 'POST' })
        .then(r => r.json())
        .then(data => {
            if (data.success) {
                // Update local state
                ...
                updateContainerDisplay(channel, {
                    remaining_ml: data.remaining_ml,
                    remaining_pct: 100,
                    container_ml: data.container_ml,
                    low_warning: false
                });
                
                console.log(`CH${channel} refilled to ${data.remaining_ml} ml`);
            }
        })
...
function updateContainerDisplay(idx, data) {
    ...
    if (bar) {
        const pctVal = data.remaining_pct !== undefined ? data.remaining_pct : 
                       (data.remaining_ml / data.container_ml * 100);
        bar.style.width = `${pctVal}%`;
        bar.classList.toggle('low', data.low_warning);
    }
    
    if (text) {
        text.textContent = `${data.remaining_ml.toFixed(0)} ml`;
    }
    
    if (pct) {
        const pctVal = data.remaining_pct !== undefined ? data.remaining_pct : 
                       Math.round(data.remaining_ml / data.container_ml * 100);
        pct.textContent = `${pctVal}%`;
        pct.classList.toggle('low', data.low_warning);
    }
    
    if (remaining) {
        remaining.textContent = `${data.remaining_ml.toFixed(0)} ml`;
    }
    
    // Update local channel data
    if (channels[idx]) {
        channels[idx].containerMl = data.container_ml;
        channels[idx].remainingMl = data.remaining_ml;
        channels[idx].remainingPct = data.remaining_pct;
        channels[idx].lowVolume = data.low_warning;
    }
}
