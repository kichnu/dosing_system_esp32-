...
.params-grid{display:grid;grid-template-columns:repeat(1,1fr);gap:8px;margin-bottom:0}
...
.valid-msg.info{background:rgba(56,189,248,0.1);border:1px solid rgba(56,189,248,0.3);color:var(--accent-blue);margin-top:7px}
...
function updateChannel(idx){
    ...
    else if(isDur&&ch.dailyDose>600){validMsg.classList.add('err');validTxt.textContent=`Duration ${ch.dailyDose}s > max 600s`;icon='<circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/>';saveBtn.disabled=true;ch.state='invalid';}
